	...
	public static void main(String[] args) {
		...
		if(Howenter==3) {
			...
			if (rock_room==1){
				...
				if ( rock[erved - 1] == 0) {
					rock[erved - 1] = 1;
					Rentalgoods rg = new Rentalgoods(rock_room,erved); //Amenities 클래스 am1은 이름을 지어줌
...
//		}
	...
	public static void main_cash(String[] args) {
		totalmoney tomoney = totalmoney.getInstance();
		System.out.println("현재 금액은" + tomoney.getAllmoney() +"입니다");
	...
	public static void main_gift_card1(String[] args){                   // main1
	   ...
	   //  -------------------------------- 나중에 수정하기^^^^^^^
		...
		if(gift_cash_2 == tomoney.getAllmoney()) {
			System.out.println(totalmoney.getInstance() + " - " + gift_cash_2 + " = " + "0" +"모두 결제가 완료되었습니다.");
		}else {
			System.out.println("------------------------------------------------------------");
